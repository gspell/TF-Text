...
class CNN:
  ...
  def __init__(self, x, filter_sizesnetwork_architecture, drop_keep=.75, learning_rate=1e-4):
  ...
  def _initialize_network_params(self, n_input, n_hidden_1, n_hidden_2, n_classes):
    self.weights = {'wc1': tf.Variable(tf.random_normal([n_input, n_hidden_1])),
               'h2': tf.Variable(tf.random_normal([n_hidden_1, n_hidden_2])),
               'out': tf.Variable(tf.random_normal([n_hidden_2, n_classes])) }
    self.biases = {'bc1': tf.Variable(tf.random_normal([n_hidden_1])), 
              'bc2': tf.Variable(tf.random_normal([n_hidden_2])),
